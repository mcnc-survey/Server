CREATE TABLE SURVEYS (
     ...
     CREATED_BY VARCHAR(40) NOT NULL,     -- 생성자 UUID
     CREATED_AT TIMESTAMP NOT NULL,       -- 생성일, 기본값은 현재 시간
     MODIFIED_BY VARCHAR(40) NOT NULL,    -- 수정자 UUID
     MODIFIED_AT TIMESTAMP NOT NULL       -- 수정일, 기본값은 현재 시간
);
...
COMMENT ON COLUMN QUESTIONS.ORDER_NUMBER IS '질문 순서';
COMMENT ON COLUMN QUESTIONS.COLUMNS IS '개수의 제한이 없어 TEXT로 설정';
COMMENT ON COLUMN QUESTIONS.ROWS IS '데이터 개수의 제한이 없어 TEXT로 설정';
