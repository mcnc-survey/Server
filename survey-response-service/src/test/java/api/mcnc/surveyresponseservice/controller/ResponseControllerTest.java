...
class ResponseControllerTest {
  ...
  void 사용자_응답_조회() throws Exception {
    ...
    Map<Integer, ResponseResult> response = Map.of(1, ResponseResult.builder().id("3af1fbc5-4c4f-4fd1-bf48-15788c0eadfb").response("만족").etc(null).build());
      ...
      .andDo(
        document("getAllResponseByRespondent",
          ...
          responseFields(
            ...
            fieldWithPath("body.responseResult.1.etc").type(NULL).description("1번 질문의 나의 기타 응답").optional()
  ...
  void 사용자_응답_저장() throws Exception {
    ...
    QuestionResponse qr1 = new QuestionResponse("uuid-32", SINGLE_CHOICE, 1, true, "그렇다", null);
    QuestionResponse qr2 = new QuestionResponse("uuid-32", MULTIPLE_CHOICE, 2, true, "피자", "치킨");
    QuestionResponse qr3 = new QuestionResponse("uuid-32", SHORT_ANSWER, 3, true, null, "집에 가고 싶다");
    QuestionResponse qr4 = new QuestionResponse("uuid-32", LONG_ANSWER, 4, true, "집에 가고 싶다 라고 말하면 집에 갈 수 있다는 것에 대해서 말하다 보면 언젠가 집에 가지 않을까 하는 생각", null);
      ...
      .andDo(
        document("saveResponse",
          ...
          requestFields(
            ...
            fieldWithPath("responses[].response").type(STRING).description("답변").optional(),
            fieldWithPath("responses[].etc").type(STRING).description("기타 응답").optional()
          ),
          pathParameters(
  ...
  void 사용자_응답_수정() throws Exception {
    ...
    ResponseUpdateRequest updateRequest = new ResponseUpdateRequest(
      List.of(
        new QuestionResponseUpdate("87494fba-cc90-4a8f-a38a-4744664c3bea", SINGLE_CHOICE, true, "아니다", null),
        new QuestionResponseUpdate("87494fba-cc90-5a8f-b48c-7743644c351a", MULTIPLE_CHOICE, true, null, "탕수육")
      )
    );
      ...
      .andDo(
        document("updateResponse",
          ...
          requestFields(
            ...
            fieldWithPath("responses[].questionType").type(STRING).description("질문 유형"),
            fieldWithPath("responses[].isRequired").type(BOOLEAN).description("필수 여부"),
            fieldWithPath("responses[].response").type(STRING).description("수정된 답변").optional(),
            fieldWithPath("responses[].etc").type(STRING).description("수정된 기타 답변").optional()
          ),
          pathParameters(
            parameterWithName("surveyId").description("설문지 ID")
          )
